...
Cree la carpeta `.streamlit` y el archivo `secrets.toml` dentro de la raíz del proyecto para definir la contraseña de acceso local y las claves de API:
...
# ── Proveedor de clasificación (tono/tema/subtema) ──────────────────────────
# Opción A (recomendada): API key de AgentRouter (https://agentrouter.org) —
# clasificación vía deepseek-v4-flash (u otro modelo soportado). Basta con
# agregar DEEPSEEK_API_KEY para activar el modelo deepseek-v4-flash.
DEEPSEEK_API_KEY = "tu_key_de_agentrouter"
# DEEPSEEK_MODEL = "deepseek-v4-flash"        # opcional: override del modelo
# DEEPSEEK_BASE_URL = "https://agentrouter.org/v1"  # opcional: endpoint propio

# Opción B (por defecto si no hay DEEPSEEK_API_KEY): OpenAI.
# OPENAI_API_KEY = "sk-..."
# OPENAI_CLASIF_MODEL = "gpt-4.1-nano-2025-04-14"  # opcional: override del modelo

# ── Embeddings (SIEMPRE OpenAI — text-embedding-3-small) ─────────────────────
# Necesaria en ambos casos: los embeddings de agrupación van a OpenAI.
OPENAI_API_KEY = "sk-..."
EOF
```

> **Nota sobre proveedores:** la clasificación (Tono IA, Tema, Subtema) usa **AgentRouter** (`https://agentrouter.org/v1`, con `DEEPSEEK_API_KEY` = tu key de AgentRouter) si existe `DEEPSEEK_API_KEY`; por defecto sirve `deepseek-v4-flash`. Si no existe la key, usa OpenAI con `gpt-4.1-nano-2025-04-14`. Los **embeddings** (agrupación de noticias similares) siempre usan OpenAI (`text-embedding-3-small`), por lo que `OPENAI_API_KEY` es obligatoria en ambos casos. Overrides sin redeploy: `OPENAI_CLASIF_MODEL` gana sobre cualquier proveedor; `DEEPSEEK_MODEL` aplica solo cuando `DEEPSEEK_API_KEY` está activa; `DEEPSEEK_BASE_URL` cambia el endpoint (default `https://agentrouter.org/v1`).
...
3. En la sección **Advanced Settings -> Secrets**, agregue las variables de entorno correspondientes:

```toml
password = "tu_contrasena_de_produccion"

# Clasificación vía AgentRouter (https://agentrouter.org/v1): activa deepseek-v4-flash
DEEPSEEK_API_KEY = "tu_key_de_agentrouter"
# DEEPSEEK_MODEL = "deepseek-v4-flash"   # opcional

# Embeddings y fallback de clasificación (siempre requerida)
OPENAI_API_KEY = "sk-..."
```
